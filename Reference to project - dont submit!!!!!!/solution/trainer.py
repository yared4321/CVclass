...
class Trainer:
    ...
    def train_one_epoch(self) -> tuple[float, float]:
        ...
        for batch_idx, (inputs, targets) in enumerate(train_dataloader):
            ...
            # compute loss w.r.t criterion
            loss = self.criterion(outputs, targets)
            # backward pass
            loss.backward()
            # optimizer step
            self.optimizer.step()
            # update avg loss + accuracy
            total_loss = total_loss + loss
            avg_loss = total_loss.item() / (batch_idx + 1)
            
            preds = outputs.argmax(dim=1)
            correct_labeled_samples = correct_labeled_samples + (preds == targets).sum().item()
            nof_samples = nof_samples + inputs.shape[0]
