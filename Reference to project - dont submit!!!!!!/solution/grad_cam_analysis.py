...
def get_grad_cam_visualization(test_dataset: torch.utils.data.Dataset,
    ...
    dataset = input_tensor.dataset

    # Get a random sample
    sample = next(iter(input_tensor))

    # Extract the image and true label from the sampled batch
    image_sample, true_label = sample[0], sample[1]
    ...
    grayscale_cam = cam(input_tensor=image_sample, targets=targets)
    # In this example grayscale_cam has only one image in the batch:
    grayscale_cam = grayscale_cam[0, :]
    img_array = image_sample[0].numpy()
