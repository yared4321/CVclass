...
class FacesDataset(Dataset):
    ...
    def __getitem__(self, index) -> tuple[torch.Tensor, int]:
        ...
        # TODO: verify wished behavior with index from real/fake (currently real)
        if index < len(self.real_image_names):
            label = 0
            img = Image.open(os.path.join(self.root_path,'real', self.real_image_names[index]))
        else:
            index -= len(self.real_image_names)
            label = 1
            img = Image.open(os.path.join(self.root_path,'fake', self.fake_image_names[index]))
        # TODO: verify there is always a transform (or should we add transform from PIL to torch tensor?)
