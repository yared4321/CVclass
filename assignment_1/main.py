...
def preparatory_steps():
    
    print("START\n\n\n\n")
    # loading data with perfect matches
    src_img, dst_img, match_p_src, match_p_dst = load_data()

    print(f"number of maching points: {match_p_dst[0].shape} points\n\n\n")
    assert match_p_dst[0].shape == match_p_dst[0].shape, "Number of source and destination points match"

    # images with points
    plt.subplot(1,2,1)
    plt.suptitle("matching points")
    plt.imshow(src_img, cmap='gray')
    plt.scatter(match_p_src[0], match_p_src[1], s=5, c='red', marker='o')
    plt.title(f'Image Source with Points')
    plt.axis('off')
    ...
    print(f"number of maching points with outliers: {match_p_dst[0].shape} points\n\n\n")
    assert match_p_dst[0].shape == match_p_dst[0].shape, "Number of source and destination points match"

   # images with points
    plt.subplot(1,2,1)
    plt.suptitle("matching points and outliers")
    plt.imshow(src_img, cmap='gray')
    plt.scatter(match_p_src[0], match_p_src[1], s=5, c='red', marker='o')
    plt.title(f'Image Source with Points')
    plt.axis('off')
